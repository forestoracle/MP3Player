...
class TDA7419 {
  public:
    ...
    /** 
      ...
      */
    void setInput(uint8_t input, uint8_t gain, bool auto_zero);
    ...
    /**
      ...
      */
    void setMainLoudness(uint8_t attenuation, uint8_t center_frequency, bool high_boost, bool softstep);

    /**
         @brief Soft Mute, Pin Influence for Mute, Soft Mute Time, Soft Step Time, Clock Fast Mode

         @param soft_mute Soft Mute: 0 - on, 1 - off
         @param pin Pin Influence for Mute: 0 - Pin&IIC, 1 - IIC
         @param soft_mute_time Soft Mute Time: 0 - 0.48ms, 1 - 0.96ms, 3 - 123ms
         @param softstep_time Soft Step Time: 0...7 (0.160ms, 0.321ms, 0.642ms, 1.28ms, 2.26ms, 5.12ms, 10.24ms, 20.48ms)
         @param clock_fast_mode Clock Fast Mode: 0 - on, 1 - off
    */
    void setSoftMute(uint8_t soft_mute, uint8_t pin, uint8_t soft_mute_time, uint8_t softstep_time, uint8_t clock_fast_mode);

    /// Volume (Gain/Attenuation, Soft Step) ///
    void setVolume(uint8_t volume, uint8_t softstep);  // Volume

    /**
         @brief Контроль затухания (attenuation)

         @param attenuation Gain/Attenuation: 0...80 (80...0) 0 - mute
         @param softstep Soft Step: 0 - on, 1 - off
         */
    void setAttenuation_LF(uint8_t attenuation, uint8_t softstep);         // Volume   LF
    void setAttenuation_RF(uint8_t attenuation, uint8_t softstep);         // Volume   RF
    void setAttenuation_LT(uint8_t attenuation, uint8_t softstep);         // Volume   LT
    void setAttenuation_RT(uint8_t attenuation, uint8_t softstep);         // Volume   RT
    void setAttenuation_Subwoofer(uint8_t attenuation, uint8_t softstep);  // Volume   SUBWOOFER
    void setAttenuation_Mixing(uint8_t attenuation, uint8_t softstep);     // Volume   MIXING
    ...
    /** 
         ...
         @param ref_out_select Reference Output Select: 0 - External Vref = 4V, 1 - Internal Vref = 3.3V
    */
   ...
   */
    void setSub_M_B(uint8_t subwoofer, uint8_t middle, uint8_t bass, uint8_t bass_dc_mode, uint8_t smoothing_filter);  // (0...3, 0...3, 0...1, 0...1)
     ...
     */
    void setMix_Gain_Eff(uint8_t left_front, uint8_t right_front, uint8_t enable, uint8_t subwoofer_enable, uint8_t gain_effect);

    /**
         @brief Настройка анализатора спектра.

         @param q_factor Filter Q Factor (3.5, 1.75)
         @param reset_mode Reset mode (IIC, Auto)
         @param source Source (Bass, In gain)
         @param run Run (on, off)
         @param reset Reset (on, off)
         @param clock_source Clock source (internal, external)
         @param coupling_mode Coupling mode (DC Coupling (without HPF), AC coupling after In gain, DC Coupling (with HPF), AC coupling after Bass)
   */
